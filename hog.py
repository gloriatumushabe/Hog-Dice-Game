...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    counter, total , total_ones =0, 0, 0
    ...
    if total_ones == 0:
        return total
    return min(total_ones, 11 - num_rolls)
...
def free_bacon(opponent_score):
    ...
    # BEGIN PROBLEM 2
    score = max( opponent_score%10 + 1, opponent_score//10 + 1)
...
def get_next_prime(b):
    a = b + 1
    while not isPrime(a):
        a = a + 1
    return a

def next_prime(currentscore):
    if isPrime(currentscore):
        return get_next_prime(currentscore)
    return currentscore
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        ...
        return next_prime(roll_dice(num_rolls, dice))
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    dice_number, highest, max_score = 1, -1, -1
    average = make_averaged(roll_dice, num_samples)
    while dice_number <= 10:
        score = average(dice_number, dice)
        if max_score < score:
            highest = dice_number
            max_score = score
...
def final_strategy(score, opponent_score):
    """ This strategy combines a set of beneficial strategies to enable the player to beat the opponent.
    These include: Predicting upcoming swaps when rolling zero dice or after hog wild and trying to 
    force the swap  of scores only when the opponent score is greater and rolling zero dice when it triggers 
    a better score against the opponent
    """
    # BEGIN PROBLEM 11
    
    if score + free_bacon(opponent_score) > 100 and not (free_bacon(opponent_score)+score)//2 == opponent_score:
        return 0
    if (free_bacon(opponent_score)+score)*2 == opponent_score:
        return swap_strategy(score, opponent_score, 11, 4)
    if (opponent_score + score + free_bacon(opponent_score))% 7== 0:
        return 0
    if (score + opponent_score)% 7== 0: 
        return swap_strategy(score, opponent_score,7, 4) 
    if (free_bacon(opponent_score) + score) * 2 == opponent_score + 1:
        return 0
